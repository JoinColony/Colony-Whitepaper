...
\subsection{EternalWallet Contract}
...
When a colony is being upgraded, we must also account for any token balances the colony has. If the colony has balances in a large number of tokens, sending each and every one of these token balances could conceivably result in the gas limit preventing the upgrade process being completed in a single transaction. This is a major reason for the \code{EternalWallet}; the upgrade process is handled analgously to the \code{EternalStorage}.
