...
The Colony Network consists of a collection of contracts on the Ethereum blockchain. At the core of the network will be a \code{ColonyNetwork} contract. This contract is primarily responsible for managing the reputation mining process (described in Section \ref{sec:reputationmining}), but also for general control of the network --- for example, setting the fees associated with using the network, or deploying new versions of the \code{Colony} contract. These actions will be mediated by a special colony, the \rc.
