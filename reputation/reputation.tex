...
\subsection{On-chain representation of skills and domains}
In the context of reputation, domains and skills are the same, differing only in that domains are colony-specific categorisation and skills are universal categorisation. Each domain or skill that reputation can be earned in is assigned a \ascode{rep\_id} that is unique across the whole network. When a domain or skill is created, additional properties are recorded and initialised.
\[
  \ascode{rep\_id} \rightarrow 
  \begin{cases}
    \ascode{generation} &	\textnormal{total number of parents}\\
    \ascode{parent\_n\_id} &	\textnormal{the \ascode{rep\_id} of the $n^{th}$ parent, where $n$ is a power of two. All  }\\
    \ascode{children}\left[\cdots\right] &	\textnormal{array of \ascode{rep\_id}s of all children}\\
    \ascode{n\_children} &	\textnormal{total number of children}
  \end{cases}
\]
Upon creation, \ascode{children[]} and \ascode{n\_children} are empty.
