...
In the context of reputation, domains and skills are the same, differing only in that domains are colony-specific categorisation and skills are universal categorisation. For each domain or skill that reputation can be earned in, there is a \ascode{rep\_id} that is unique. When a domain or skill is created, additional properties are recorded.
\[
  \ascode{rep\_id} \rightarrow 
  \begin{cases}
    \ascode{parent\_id} &	\textnormal{the \ascode{rep\_id} of the parent domain or skill}\\
    \ascode{parent\_n\_id} &	\textnormal{the \ascode{rep\_id} of the $n^{th}$ parent}\\
