...
There are three ways to earn reputation in a colony. The first is being involved with a successfully completed task (detailed in Section \ref{sec:earning-rep-from-task}) and the second is through the dispute process (described in Section \ref{sec:earning-rep-in-disputes}). In both of these cases, the user has been a productive member of the colony and is rewarded accordingly. The third way to earn reputation is upon the creation of a colony and the associated bootstrapping process (see Section \ref{sec:bootstrapping-rep}). In this case, the user receiving the reputation is the colony creator or an acquaintance, and the reputation should be seen as a representation of the trust they have for their colleague. 
...
When a new domain is created in a colony, it has no reputation in it, like a new colony, and so we must consider how to bootstrap the domain as well. We do not wish to allow the creation new reputation here, as this would devalue reputation already earned elsewhere in the colony. 
