...
\begin{equation}\label{eq:miner-weighting}
 w_n = \left(1 - e^{\frac{t_n}{T}}\right) %\left( 1 - \frac{n-1}{N} \right)
\end{equation}
where $t_n$ is a number of blocks that the $n^{th}$ miner has staked their tokens for and $T$ %and $N$ are
is a normalising constant, set to a number of blocks representing 3 months.%, and $N$ is set to twice the length of the list of submitters - in our case $N=24$.
