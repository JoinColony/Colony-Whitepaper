...
It is possible that the same hash has been submitted twice, but with a disagreement about the number of leaves it contained. In this situation, the users are required to submit a Merkle proof for the last leaf in their tree. We are able to tell if a submitted proof corresponds to the last leaf in the tree, as any hashes on the right must be hashed with themselves, and any hashes on the left must not be. The structure of the Merkle proof also allows us to tell the index of this last leaf; if it corresponds to either of the submitted states, that one is correct.
...
If every hour $A$ reputation is awarded in \rc, % and if there is a total $R_0$ \rc\ reputation, then at the next cycle we have reputation
%\[
%R_1^\prime = R_0 (1 - e^{-k}) + A
%\]
%So $R_1^\prime - R_0 = A - R_0e^{-k}$ and this should be $3/4$ of the total new rep %awarded. In other words
%\[
%R_1 -R_0 \stackrel{!}{=} 4/3 (A - R_0 e^{-k})
%\]
%Therefore the new mining rep ($R_1 - R_0$) should be
%\[
%1/3 ( A - R_0 e^{-k} )
%\]

%... besides, I think we should just say:
we want
\[
 A \stackrel{!}{=} 3 R_M
\]
where $R_M$ is the mining reward. $A$ (and thus $R_M$) is calculated on a one-month moving average. 
It's value is submitted by us as an oracle ... at least at first... because how would we do that on-chain?

Weighting: We want 10 submissions per cycle. The submitters are ordered in a list (position = $n$ starting at 0) and given the weighting
\[
W_n = \left(1 - e^{\frac{t}{T}}\right) \left( 1 - \frac{n}{N} \right)
\]

If we set N = 18, then the last submitter gets half the reward of the first. 
