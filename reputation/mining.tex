...
If we cannot figure out what the correct submission is within the given timeframe, then the rep update process stalls... that's about it though. Very expensive attack and only effect is to delay one update.



\subsection{Costs and Rewards of Mining}
-token holders must stake in order to be miners\\
-reputational reward comes from tokens earned just like task payout
-how much rep exactly you get (siilar to 5 star rating for work) determined by how long you lock your tokens. Short term = 3 stars, long term 5 stars.\\
-how many tokens are paid out to miners must vary as the price of the token changes. Idea: make it scale with the total paid out in other domains of root colony. If feasible, use one month rolling average. Prevent miners overpowering rest of root colony with too much rep.
