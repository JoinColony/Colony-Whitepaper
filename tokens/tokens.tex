...
After the Colony Network Contract is deployed, the first colony created will be the \rc, and rather than deploying a token contract alongside, it will use the existing token contract. Tokens in this Colony will be CLNY. Functionality will be added to this contract as required, through the mechanism described in Appendix \sec{app:clnyupgrade}. The ability to add functionality will never lie with the developers, but a multisignature contract made up of members of the community, and then subsequently the \rc itself.
