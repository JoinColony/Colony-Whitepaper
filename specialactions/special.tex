...
\subsection{Awarding reputation bonuses}
...
Of course, it is possible that a colony will want to engage in some behaviour that we haven't forseen, that could be implemented in a contract outside the control of the Colony Network. To that end, we wish to have a mechanism by which a colony can create an arbitrary transaction on the blockchain to interact with contracts and tokens without requiring the network to explicitly support them. As they are powerful, such transactions should be rare occurrences with high threshold requirements.
